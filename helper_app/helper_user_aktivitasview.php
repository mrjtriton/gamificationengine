...
$user_aktivitas_view = NULL; // Initialize page object first

class cuser_aktivitas_view extends cuser_aktivitas {
	...
	// Page object name
	var $PageObjName = 'user_aktivitas_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["user_id"] <> "") {
				...
				$sReturnUrl = "helper_user_aktivitaslist.php"; // Return to list
			}
			if (@$_GET["aktivitas_id"] <> "") {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "helper_user_aktivitaslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "helper_user_aktivitaslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($user_aktivitas_view)) $user_aktivitas_view = new cuser_aktivitas_view();

// Page init
$user_aktivitas_view->Page_Init();

// Page main
$user_aktivitas_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$user_aktivitas_view->Page_Render();
?>
<?php include_once "helper_header.php" ?>
<script type="text/javascript">

// Page object
var user_aktivitas_view = new ew_Page("user_aktivitas_view");
user_aktivitas_view.PageID = "view"; // Page ID
var EW_PAGE_ID = user_aktivitas_view.PageID; // For backward compatibility

// Form object
var fuser_aktivitasview = new ew_Form("fuser_aktivitasview");

// Form_CustomValidate event
fuser_aktivitasview.Form_CustomValidate = 
...
fuser_aktivitasview.ValidateRequired = true;
<?php } else { ?>
fuser_aktivitasview.ValidateRequired = false; 
<?php } ?>
...
<?php $user_aktivitas_view->ExportOptions->Render("body") ?>
<?php
	foreach ($user_aktivitas_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $user_aktivitas_view->ShowPageHeader(); ?>
<?php
$user_aktivitas_view->ShowMessage();
?>
<form name="fuser_aktivitasview" id="fuser_aktivitasview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($user_aktivitas_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $user_aktivitas_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="user_aktivitas">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($user_aktivitas->user_id->Visible) { // user_id ?>
	<tr id="r_user_id">
		<td><span id="elh_user_aktivitas_user_id"><?php echo $user_aktivitas->user_id->FldCaption() ?></span></td>
		<td<?php echo $user_aktivitas->user_id->CellAttributes() ?>>
<span id="el_user_aktivitas_user_id" class="form-group">
<span<?php echo $user_aktivitas->user_id->ViewAttributes() ?>>
<?php echo $user_aktivitas->user_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($user_aktivitas->aktivitas_id->Visible) { // aktivitas_id ?>
	<tr id="r_aktivitas_id">
		<td><span id="elh_user_aktivitas_aktivitas_id"><?php echo $user_aktivitas->aktivitas_id->FldCaption() ?></span></td>
		<td<?php echo $user_aktivitas->aktivitas_id->CellAttributes() ?>>
<span id="el_user_aktivitas_aktivitas_id" class="form-group">
<span<?php echo $user_aktivitas->aktivitas_id->ViewAttributes() ?>>
<?php echo $user_aktivitas->aktivitas_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($user_aktivitas->poin->Visible) { // poin ?>
	<tr id="r_poin">
		<td><span id="elh_user_aktivitas_poin"><?php echo $user_aktivitas->poin->FldCaption() ?></span></td>
		<td<?php echo $user_aktivitas->poin->CellAttributes() ?>>
<span id="el_user_aktivitas_poin" class="form-group">
<span<?php echo $user_aktivitas->poin->ViewAttributes() ?>>
<?php echo $user_aktivitas->poin->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($user_aktivitas->insert_at->Visible) { // insert_at ?>
	<tr id="r_insert_at">
		<td><span id="elh_user_aktivitas_insert_at"><?php echo $user_aktivitas->insert_at->FldCaption() ?></span></td>
		<td<?php echo $user_aktivitas->insert_at->CellAttributes() ?>>
<span id="el_user_aktivitas_insert_at" class="form-group">
<span<?php echo $user_aktivitas->insert_at->ViewAttributes() ?>>
<?php echo $user_aktivitas->insert_at->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fuser_aktivitasview.Init();
</script>
<?php
$user_aktivitas_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$user_aktivitas_view->Page_Terminate();
?>
